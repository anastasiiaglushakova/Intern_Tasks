...
void write_wav(const char* filename, const std::vector<int16_t>& data,
  ...
  uint32_t riff_chunk_size = 4 + (8 + fmt_chunk_size) +
                             (8 + data_chunk_size);  // Общий размер RIFF блока
  uint16_t audio_format = 1;                         // PCM формат
  uint16_t num_channels = 1;                         // Моно
  uint16_t bits_per_sample = 16;
  uint32_t byte_rate = sample_rate * num_channels * bits_per_sample /
...
std::vector<int16_t> karplus_strong_synthesis(
    const std::vector<double>& frequencies, double sample_rate,
    double duration_sec, double decay) {
  ...
  for (double freq : frequencies) {
    ...
    for (size_t i = 0; i < total_samples; ++i) {
      ...
      int16_t out_sample = static_cast<int16_t>(
          std::max(-1.0, std::min(1.0, new_sample)) * 32767);
